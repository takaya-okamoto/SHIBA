...
- claim は**それ単体で意味が通る**ように書く。「もの/こと/それ/これ/あれ/そう」等の指示語で終えたり、主語・対象を省いたりしない。何についての事実/好み/予定なのかを claim 自体に必ず含める(例: ✗「短めのものを希望している」→ ✓「SHIBAからの返信は短めを好む」/ ✗「柴犬系が好き」→ ✓「アシスタントは柴犬のキャラクターが良いと考えている」)。後で文脈なしに読んでも分かる粒度にする。
